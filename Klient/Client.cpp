        ...
        if (buffer[0] != 0) {
            ...
            switch ((buffer[0] >= 48 && buffer[0] <= 57 ? (BufferOutput) (int) (serverOutput - 48)
                ...
                case BufferOutput::SendMessageUnsuccessful:
                    ...
                    priznakMenu = 3;
                    break;
                case BufferOutput::DisconnectSucc:
                    std::cout << "DISCONNECTETTO" << std::endl;
                    priznakMenu = 6;
                    break;
            }
...
void Client::ProcessBufferMessage(char *buffer) {
    ...
    sizeMena = std::string(&buffer[pocitadlo], &buffer[pocitadlo + 1]);
    pocitadlo += 1;
    meno = std::string(&buffer[pocitadlo], &buffer[pocitadlo + (int)sizeMena[0]]);
    pocitadlo += (int)sizeMena[0];

    string = std::string(&buffer[pocitadlo], &buffer[pocitadlo + (BUFF_SIZE - pocitadlo - 1 - (int)sizeMena[0])]);
    pocitadlo += countOfCharsToWrite;
    countOfWords++;
    ...
    std::cout << "SPRAVA " << "OD POUZIVATELA: " << meno << " --> " <<  string << std::endl;
}

std::string Client::DecryptMessage(std::string sprava) {
