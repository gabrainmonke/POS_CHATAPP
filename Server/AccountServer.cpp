...
std::string AccountServer::SendChatMessageToReceiver(std::string message, std::string meno) {
    char charakter = static_cast<char>(BufferOutput::SendingMessage);
    std::string Message(1, charakter);
    std::string size = std::to_string(meno.size());
    Message += meno.size();
    Message += meno;
    Message += message;
    return Message;
...
std::string AccountServer::EncryptPass(std::string pass) {
    // Nasa implementacia cezarovskej sifry
...
std::string AccountServer::SendSuccessDisconnect() {
    char charakter = static_cast<char>(BufferOutput::DisconnectSucc);
    std::string SendMessageSucc(1, charakter);
    return SendMessageSucc;
}
